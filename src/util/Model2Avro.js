...
// might need to create a method to output a json schema doc
// this.domain.getDomain().schema.obj

module.exports = class Model2Avro {
    ...
    _parse(key, object) {
        let shit = this._parseDelegated('user_id', object);
    ...
    _parseDelegated(key, object) {
        ...
        if (object.type === 'parsedTestConfig' || object.type === 'undefined' || object.type === undefined) {
            console.log('=========================');
        }
        ...
        else if (object.type === undefined || (type === 'function' && !this._isPrimitive(object))) {
            return false;
        }

        if (type === 'object' && !Model2Avro._isEmptyObject(object)) {
    ...
    _parseObject(key, object) {
        ...
        let result = Model2Avro._addNull(type);
        result.name = key;
    ...
    _parsePrimitive(key, primitive) {
        ...
        mappedType = Model2Avro._addNull(mappedType);
    ...
    _parsePrimitiveObject(key, primitiveObject) {
        ...
        if (!Model2Avro._isRequired(primitiveObject)) {
            mappedType = Model2Avro._addNull(mappedType);
            return Object.assign({ name: key }, mappedType);
        }
    ...
    _isPrimitive(object) {
        if (Model2Avro._isEmptyObject(object) || Model2Avro._isEmptyArray(object)) {
            return true;
        }
